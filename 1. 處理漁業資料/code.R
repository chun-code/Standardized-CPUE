...
#cluster：後續新增的欄位，代表漁獲組成，可以根據每一航次中11種漁獲物的組成比例計算取得。透過每一航次的漁獲組成可以了解漁民的捕撈策略，例如：這趟出海主要追蹤鎖管進行捕撈，或是這趟航程物種組成多樣，可能是因為作業位置偏離鎖管棲息的海域。此數據可視為影響CPUE因子之一。  
#SST(boat)船測水溫：填補遺漏值。  
...
fdata_sst = fdata_env
missDummy = function(t) #將SST_boat為na值標示出來
{
