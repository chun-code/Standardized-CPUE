...
####(5) 比較標準化CPUE和名目CPUE----
#尚為經過標準化的CPUE稱為名目CPUE(nominal CPUE)，也就是直接將捕獲量和努力量相除(catch/effort)。
#多數研究考量了漁業因素(如：燈泡總瓦數、捕撈策略)、環境因素(如：海水溫)等進行標準化CPUE，認為這樣的結果可以排除作漁撈作業造成的偏差，因此較具參考性。
#以下比較standard CPUE和nominal CPUE，藉此了解數值的差異。
